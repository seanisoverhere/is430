...
const scoreAggregate = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
    let { industry, sales, otherIncome, costOfGoodsSold, operatingExp,
        interestExp, currentAssets, currentLiabilities, inventory, longTermLiabilities } = req.body

    const companyFinHealthScore = await bizAggregatedScore(industry, sales, otherIncome, costOfGoodsSold, operatingExp,
        interestExp, currentAssets, currentLiabilities, inventory, longTermLiabilities)
